...
const USER_SESSION_NAME = "nutriscribe_bn_scholar";
...
class Session
{
    ...
    public static function getUser(PDO $pdo)
    {
        ...
        $query = $pdo->prepare("SELECT scholar_infos.id as scholar_info_id,scholar_infos.photo,scholar_infos.firstname,scholar_infos.middlename,scholar_infos.lastname,scholar_infos.email,scholar_infos.address,scholar_infos.phone,scholar_accounts.id,scholar_accounts.username FROM scholar_accounts INNER JOIN scholar_infos ON scholar_accounts.id = scholar_infos.scholar_account_id WHERE scholar_accounts.id = ?");
